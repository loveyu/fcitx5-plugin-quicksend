...
- **测试覆盖**：`src/test` 已有腾讯 ASR 客户端签名单测——`TencentV2SigningTest`（V2 签名串格式 + HMAC-SHA1/Base64 与 JDK 对拍）、`TencentAsrV1SigningTest`（V1 文档示例签名串格式 + filter_punc 字典序）。无 instrumented 测试。`SendActionBuilder`（纯算法）、`ProxyConfig.fromUri`、`KeyNameMapping` 等仍适合补单测。
