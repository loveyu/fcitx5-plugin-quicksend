...
- 资源：`res/values` 与 `res/values-night` 双色（悬浮窗 Service 上下文随系统 uiMode 切日夜）；Compose 颜色默认走 `QuickSendTheme` baseline 配色，自定义语义色（如延迟段琥珀色）也必须成对定义日夜资源并经共享样式入口读取。无 XML 布局、无 ViewBinding。
